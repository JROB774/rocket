...
CS_PUBLIC_SCOPE::gfx
{
    ...
    CS_API void EndRenderFrame()
    {
        ...
        // We only blit the the main framebuffer to the screen if debug mode is not enabled.
        // If it is then we instead want to blit it to an ImGui window to allow for all of
        // the docking, windowing, etc. that comes with the debug mode systems.
        if(!IsDebugMode())
        {
            s32 srcX0 = 0;
            s32 srcY0 = 0;
...
CS_PUBLIC_SCOPE
{
    ...
    void Asset<gfx::Texture>::DoDebugView()
    {
        ...
        ImVec2 cursorPos = imagePos + ImVec2(0.0f, imageSize.y * 0.5f);
        cursorPos.x = roundf(cursorPos.x);
