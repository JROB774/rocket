...
CS_PUBLIC_SCOPE
{
    ...
    CS_API Vec2 GetScreenMousePos()
    {
        ...
        f32 windowWidth = (IsDebugMode()) ? CS_CAST(f32, GetDebugGameViewport().w) : CS_CAST(f32, GetWindowWidth());
        f32 windowHeight = (IsDebugMode()) ? CS_CAST(f32, GetDebugGameViewport().h) : CS_CAST(f32, GetWindowHeight());
        f32 screenWidth = GetScreenWidth();
        f32 screenHeight = GetScreenHeight();

        // If we're in debug mode then extra caclulations are needed to handle the game viewport.
        if(IsDebugMode())
        {
            Rect gameViewport = GetDebugGameViewport();
            screenMouse.x = csm::MapRange(screenMouse.x, gameViewport.x,gameViewport.x+gameViewport.w, 0.0f,windowWidth);
            screenMouse.y = csm::MapRange(screenMouse.y, gameViewport.y,gameViewport.y+gameViewport.h, 0.0f,windowHeight);
        }
    ...
    {
        if(DoesDebugUiWantMouseInput()) return false;
